...
def find_missing_albums(all_albums):
    ...
    # Owned titles are normalised so near-miss spellings don't look "missing".
    from update_lastplayed import _fold_accents

    def akey(name):
        return _normalize_for_match(_fold_accents(name))

    artist_counts = {}   # grouped key -> count
    artist_display = {}  # grouped key -> display name
    owned_titles = {}    # grouped key -> set of normalised titles
    for a in all_albums:
        k = akey(a["artist"])
        artist_counts[k] = artist_counts.get(k, 0) + 1
        artist_display.setdefault(k, a["artist"])
        owned_titles.setdefault(k, set()).add(_normalize_for_match(a["title"]))
    ...
    top_owned = [k for k, _ in sorted(artist_counts.items(), key=lambda x: -x[1])[:15]]
    top_played = []
    heatmap_file = SITE_DIR / "heatmap_data.json"
    if heatmap_file.exists():
        try:
            hist = json.loads(heatmap_file.read_text(encoding="utf-8"))
            top_played = [akey(e["artist"]) for e in hist.get("artists", [])
                          if akey(e.get("artist", "")) in artist_counts][:15]
        except Exception:
            pass
    seen = set()
    candidates = []
    for k in top_owned + top_played:
        if k not in seen:
            seen.add(k)
            candidates.append(k)
    top_artists = [(artist_display[k], artist_counts.get(k, 0)) for k in candidates[:25]]
    ...
    for artist_name, owned_count in top_artists:
        ...
        if cache_key in cache:
            ...
            if cached.get("v") == 2 and time.time() - cached.get("ts", 0) < 30 * 86400:
                ...
                owned = owned_titles.get(akey(artist_name), set())
